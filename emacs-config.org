...
It does not run interactive programs that present themselves visually well (i.e. vim, top, less, etc), but most of those are already setup in ~eshell-visual-commands~, and will open in a new term buffer. It's also not great at piping, and other unix-y type things.
...
Add Eat terminal emulator.  Not sure why, but had to run =M-x eat-compile-terminfo= to get rid of ~unknown terminal~ errors.
...
*** TODO still a little janky.  figure out how to disable bash =C-r=, or how to get out of it.  figure out why copy paste doesn't work quite right.
...
Add claude-code.el to integrate Claude Code.
