...
Improve the modeline, adding nice icons.  Requires running `M-x all-the-icons-install-fonts` to get the icons.
...
*** TODO Still getting weird symbols in modeline after running `M-x all-the-icons-install-fonts`
...
All kinds of org-mode setup: basic installation and setup and keybindings, update TODO flow, refile targets, tag shortcuts, custom org-agenda commands, and capture templates for quick keybindings to capture certain things, like todos.
...
Add the ability to execute code for specific languages from org files.
...
Add shortcuts for adding code snippets for specific languages to org files.
...
***** TODO This isn't working properly.  The doc window and flycheck sideline messages aren't showing.
...
Adds typescript support to emacs and enables lsp features for typescript-mode.
...
**** TODO Emacs 29+ has built-in `typescript-ts-mode`, so this can probably go away.
...
*** TODO The completion pop-up causes the shift and it really hurts my eyes.
